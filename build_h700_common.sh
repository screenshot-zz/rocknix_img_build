...
get_latest_version() {
    ...
    if [[ "$IS_BACKUP_REPO_ENABLED" == "true" ]]; then
        ...
        if [[ -n "$download_url" ]]; then
            ...
            echo -e "\033[1;33m⚠️ 未找到符合 $device_pattern*$file_suffix 的文件\033[0m"
            return 1
        fi
	...
	# 过滤出 H700*.img.gz 的链接（排除 .sha256 文件）
	download_url=$(echo "$assets" | grep "H700" | grep "img\.gz$")
	# 输出结果
	echo "最新 H700.img.gz 文件的下载地址是: $download_url"
}
